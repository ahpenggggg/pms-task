...
const AddPostModal: React.FC<AddPostModalProps> = ({ isOpen, onClose }) => {
  ...
  const createPostMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      // Remove all cached data for these queries
      queryClient.removeQueries({ queryKey: ['myPosts'] });
      queryClient.removeQueries({ queryKey: ['allPosts'] });
      queryClient.removeQueries({ queryKey: ['totalPostsCount'] });
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={handleClose} title="Add A Post" size="lg">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex gap-3 justify-center mt-6">
            ...
            >
              {createPostMutation.isPending ? 'Adding...' : 'Add'}
            </Button>
          </div>
        </form>
      </Modal>
